...
func TestGenesis_ExportsAndRestoresPendingState(t *testing.T) {
	...
	k := keeper.NewKeeper(cdc, storeService, tKey, stakingK, authority, nil, nil)
	...
	k2 := keeper.NewKeeper(cdc, storeService2, tKey2, stakingK, authority, nil, nil)
...
func TestGenesis_RoundTripPreservesDistinctRedelegationSources(t *testing.T) {
	...
	k2 := keeper.NewKeeper(cdc, runtime.NewKVStoreService(storeKey2), tKey2, stakingK, authority, nil, nil)
	InitGenesis(ctx2, k2, exported)
