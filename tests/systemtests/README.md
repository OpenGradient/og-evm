...
The systemtests suite is an end-to-end test suite that runs the ogd process and sends RPC requests from separate Ethereum/Cosmos clients. The systemtests for cosmos/evm use the `cosmossdk.io/systemtests` package by default. For more details, please refer to https://github.com/cosmos/cosmos-sdk/tree/main/tests/systemtests.
...
cp ./build/ogd ./tests/systemtests/binaries
cp ./build/ogd ./tests/systemtests/binaries/v0.4
```
...
go test -failfast -mod=readonly -tags=system_test ./... -run TestMempoolTxsOrdering \
  --verbose --binary ogd --block-time 3s --chain-id local-4221
```
