...
function checkTestEnv () {
  const argv = yargs(hideBin(process.argv))
      ...
      .boolean('verbose-log')
      .describe('verbose-log', 'print ogd output, default false').argv
...
function setupNetwork ({ runConfig, timeout }) {
  ...
  const spawnPromise = new Promise((resolve, reject) => {
    ...
    const serverStartedMsg = 'ogd started'
    ...
    logger.info(`Starting ogd process... timeout: ${timeout}ms`)
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start ogd timeout!')), timeout)
  })
