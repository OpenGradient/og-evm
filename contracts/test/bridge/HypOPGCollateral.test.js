...
describe("HypOPGCollateral", async function () {
  const { viem, networkHelpers } = await network.connect();
  const [owner, sender, recipient] = await viem.getWalletClients();

  async function deployHypOPGCollateralFixture() {
    ...
    const router = await viem.deployContract("HypOPGCollateral", [
      token.address,
      mailbox.address,
      ZERO_ADDRESS,
    ...
    const routerAsSender = await viem.getContractAt("HypOPGCollateral", router.address, {
      client: { wallet: sender },
    });
  ...
  it("requires ERC20 approval before locking collateral", async function () {
    const { router, routerAsSender } = await networkHelpers.loadFixture(deployHypOPGCollateralFixture);

    await viem.assertions.revertWith(
      routerAsSender.write.transferRemote([REMOTE_DOMAIN, addressToBytes32(recipient.account.address), 1000n], {
        value: DISPATCH_FEE,
      }),
      "ERC20: insufficient allowance",
    );

    assert.equal(await router.read.quoteDispatchFee([REMOTE_DOMAIN, addressToBytes32(recipient.account.address), 1000n]), DISPATCH_FEE);
  });
  ...
  it("locks collateral and dispatches on outbound transfer", async function () {
    const { mailbox, router, routerAsSender, token, tokenAsSender } =
      await networkHelpers.loadFixture(deployHypOPGCollateralFixture);
    const amount = 1_000n;
  ...
  it("releases collateral on inbound delivery", async function () {
    const { mailbox, router, token } = await networkHelpers.loadFixture(deployHypOPGCollateralFixture);
    const amount = 750n;
    const recipientBytes32 = addressToBytes32(recipient.account.address);
